...
const ADMIN_KEY = process.env.ADMIN_KEY || "TEMPLE";
const keys = new Set(["TEMP"]); // demo key for testing

// Build target URLs
function buildTargetUrl(type, term) {
  if (!term) return null;
  type = type?.toLowerCase();
  if (type === "mobile") return `https://osintx.info/API/demo.php?key=JONATHAN&type=mobile&term=${encodeURIComponent(term)}`;
  if (type === "id_number") return `https://osintx.info/API/demo.php?key=JONATHAN&type=id_number&term=${encodeURIComponent(term)}`;
  if (type === "rc") return `https://osintx.info/API/new1vehicle.php?key=JONATHAN&rc=${encodeURIComponent(term)}`;
  if (type === "cnic") return `https://osintx.info/API/pak.php?key=TEMPORARY&cnic=${encodeURIComponent(term)}`;
  if (type === "upi_id") return `https://osintx.info/API/uppi.php?key=JONATHAN&upi_id=${encodeURIComponent(term)}`;
